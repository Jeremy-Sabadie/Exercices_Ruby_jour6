text = "This is the 100th Etext file presented by Project Gutenberg, and
...
of the year 2001."
	...
	#chaque mots est intégré dans le hash avec valeur par défaut 0.
	mots = text.tr('.,"%:', ' ').downcase.split (' ')
	#pour chaque mots du texte: tr remplace les ;,": par rien, met les mots en minuscule, et met des espaces.
	mots.each do |mot|		
			frequence[mot] += 1
			#quand le mot apparait une nouvelle fois ça fait +1.		
	end

	#frequence.sort_by 
	res ={}

	dictionnaire.each do |item|
		if frequence[item]!= nil 
			res[item] = frequence[item]
		end
	end
...
=begin
if frequence.delete_if { |mot| mots !== dictionnaire['']}
